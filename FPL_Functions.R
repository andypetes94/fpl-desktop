...
player_points_plot1 <- function(data, starts_1, gameweeks_1, gameweeks_2, player_metric) {
  ...
  p <- ggplot(data, aes(x = round, y = get(player_metric), fill = name_club, color = name_club)) +
    ...
    geom_text(aes(label = get(player_metric)), size = 2, family = "Lato", color = "white") +
...
# Fix the chips processing - same issue as before
Chips <- tryCatch({
  if(!is.null(JSON_Output$chips) && 
     length(JSON_Output$chips) > 0 && 
     is.data.frame(JSON_Output$chips) &&
     nrow(JSON_Output$chips) > 0 &&
     "time" %in% names(JSON_Output$chips)) {
    
    JSON_Output$chips %>% select(-time)
  ...
  # If anything goes wrong, return empty data frame
  data.frame(
    event = integer(0),
    name = character(0),
    stringsAsFactors = FALSE
...
bar_plot_player <- function(test_df, metric1, top, player_metric, positions, GW1, GW2) {
  ...
  geom_text(aes(label = get(metric1)), size = 3, color = "white", family = "Lato") +
...
team_player_viz <- function(Minutes_Data, team_matrix, stat_matrix, y_var, text_var, col_var, color_palette, scale1) {
  ...
  ggplot(Minutes_Data, aes(x = reorder(labels, Gameweek), y = reorder_within(Position, get(y_var), name), fill = get(col_var), colour = get(col_var))) +
    ...
    geom_text(aes(label = get(text_var)), color = "#F3E9E2", fontface = "bold", family = "Lato", size = 3, show.legend = F) +
    ...
    theme(#panel.grid.major.x = element_line(color = "gray", linewidth = 1, linetype = 'dotted'),
      ...
      panel.background = element_rect(color = "#F3E9E2", fill = "#F3E9E2"),
      plot.background = element_rect(color = "#F3E9E2", fill = "#F3E9E2"),
      plot.title = element_markdown(color = "black", size = 26, family = "Lato"),
      ...
      legend.text = element_text(color = "gray30", size = 12, family = "Lato", hjust = 0),
      legend.title = element_text(color = "gray20", size = 14,  family = "Lato"),
      legend.key = element_blank(),
      legend.margin=margin(0,0,0,0),
      legend.box.margin=margin(0,0,0,0),
      #axis.text.x = element_text(color ="black", size = 10, family = "Lato", angle = 90),
      axis.text.x = element_markdown(color ="black", size = 10, family = "Lato"),
      axis.text.y = element_text(color ="gray20", size = 10, family = "Lato", hjust = 0),
      #axis.text.y = element_markdown(size = 6, color = 'black', family = "Lato", hjust = 1),
      axis.title.x = element_text(color = "black", size = 12,  family = "Lato"),
      axis.title.y = element_blank(),
      ...
      strip.background = element_blank(),
      strip.text.x = element_markdown(color = "gray20", size = 14, family = "Lato"),
      #strip.text.y.left = element_markdown(color = "gray20", size = 12, family = "Lato", , angle = 360, vjust = 0),
      legend.background = element_blank())
